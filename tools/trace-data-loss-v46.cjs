...
function sourceDatumEvidence(rule){
  ...
  for(const file of modelSourceFiles){
    ...
    lines.forEach((line,i)=>{
      if(line.includes(rule.token) && line.includes('=')) rows.push({file:rel(file),line:i+1,evidence:line.trim()});
    });
  }
  return rows;
}
...
for(const rule of modelDatumRules){
  ...
  const flow={
    ...
    phpAst:{status:'BLOCKED',reason:'No canonical model-property ADT node is currently proven for this datum.'},
  ...
  const first={
    datum:rule.datum,
    firstLoss:'PHP_AST_ADT_BOUNDARY',
    severity:'critical',
    ...
    repairTarget:'existing PHP AST/ADT vocabulary',
    downstreamRepairBlocked:true,
    reason:'The source datum exists and the semantic owner exists, but the repository does not expose a proven canonical PHP AST/ADT node carrying this datum into the owner.'
  };
...
const freeDataClassification=freeData.map(item=>({
  ...item,
  classification:item.kind==='record'
    ? (/semantic|operation|route|resource|model|response|request|type/i.test(item.evidence)?'SEMANTIC_FREE_DATA':'STRUCTURAL_DATA_CONTAINER')
    : 'SEMANTIC_FALLBACK',
  action:item.kind==='record'
    ? (/semantic|operation|route|resource|model|response|request|type/i.test(item.evidence)
      ? 'TRACE_TO_EXISTING_ADT_OWNER'
      : 'KEEP_IF_STRUCTURAL_ONLY')
    : 'MOVE_MEANING_UPSTREAM_AND_REMOVE_FALLBACK'
}));
...
const modelDataflowEdges=[
  {from:'Laravel PHP source',to:'LaravelSourceLexer.tokenize',status:'PROVEN',file:'packages/core/src/compiler/scanner/subscanners/model/modelParser.ts'},
  {from:'LaravelSourceLexer.tokenize',to:'parseModelMembers',status:'PROVEN',file:'packages/core/src/compiler/scanner/subscanners/model/modelParser.ts'},
  {from:'parseModelMembers',to:'tryParseModelProperty',status:'PROVEN',file:'packages/core/src/compiler/scanner/subscanners/model/modelMemberParser.ts'},
  {from:'tryParseModelProperty',to:'ParsedModelMembers',status:'PROVEN',file:'packages/core/src/compiler/scanner/subscanners/model/modelMemberParser.ts'},
  {from:'ParsedModelMembers',to:'ParsedModel',status:'PROVEN',file:'packages/core/src/compiler/scanner/subscanners/model/modelParser.ts'},
  {from:'ParsedModel',to:'modelAstFromParsed',status:'PROVEN',file:'packages/core/src/compiler/scanner/subscanners/model/modelCanonical.ts'},
  {from:'modelAstFromParsed',to:'ModelAst.facts',status:'PROVEN',file:'packages/core/src/compiler/scanner/subscanners/model/modelCanonical.ts'},
  {from:'ModelAst',to:'CompleteSourceAst',status:'TRACE_REQUIRED',file:'packages/core/src/types/upstream/ast.ts'},
  {from:'CompleteSourceAst',to:'RouteSyncManifest',status:'TRACE_REQUIRED',file:'packages/core/src/types/upstream/manifest.ts'}
];
...
if(modelPropertyAstBoundaryStatus==='AST_PRODUCER_AND_CONSUMER_PROVEN'){
  ...
  for(const loss of firstLossBoundary){
    ...
    loss.reason='The Laravel datum now has a canonical AST node and existing ModelFacts owner. The next unproven boundary is connection of the existing semantic owner into the high-level upstream model.';
  }
}
if(modelPropertyAstBoundaryStatus==='AST_PRODUCER_AND_CONSUMER_PROVEN'){
...
if(modelHighLevelConnectionProof.status==='UNCONNECTED'){
  add(findings,'critical','model_high_level_connection_missing',modelHighLevelConnectionProof,'Connect the existing ModelFacts/ModelAst data into the existing ModelSemanticNode/CompleteLaravelSourceModel producer boundary. Do not add duplicate top-level fields.');
}

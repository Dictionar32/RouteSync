 ...
 */
export function routeResponseKey(route: ParsedRoute): string {
 ...
 * 'user.show' → 'UserShow'
 */
export function routeToTypeName(route: ParsedRoute): string {
    if (!route.name) return 'Response'
 ...
 */
export function mapSqlTypeToMapping(sqlType: string, cast?: string): TypeMapping {
 ...
 */
export function mapSqlTypeToZod(sqlType: string, cast?: string): string {
 ...
 * Modifiers di-add di layer tergantung context
 */
export function mapSqlTypeToTs(sqlType: string, cast?: string): string {
    const mapping = mapSqlTypeToMapping(sqlType, cast)
 ...
 */
function getCastMapping(castType: string): TypeMapping | null {
    // String cast
    if (castType === 'string') {
        return { zodType: 'z.string()', tsType: 'string', baseType: 'string', isNullable: false }
    }
...
export function wrapNullableTs(typeStr: string, nullable: boolean): string {
    if (!nullable || !typeStr) return typeStr
...
export function wrapNullableZod(schemaStr: string, nullable: boolean): string {
    if (!nullable || !schemaStr) return schemaStr
 ...
 */
export function isResourceAlias(response: RuntimeAugmented<Record<string, unknown>>, knownResources: Set<string>): boolean {
    ...
    if (!knownResources.has(schemaName)) return false

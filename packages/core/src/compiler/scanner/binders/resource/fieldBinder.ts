...
export function bindField({
    ...
    return matchPhpAstValue(value, {
        methodChain: (val) => val.property === 'whenLoaded'
            ? bindWhenLoadedField(key, rawExpression, modelSymbol)
        ...
        variableReference: () => bindFallbackField(key, rawExpression),
        rawExpression: () => bindFallbackField(key, rawExpression)
    });
}

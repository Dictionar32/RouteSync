...
// V59. Exact first-loss -> producer -> owner -> repair gate.
...
function exactExistingTarget(f, loss){
  ...
  if(f.kind==='route_source_span_lost_before_ast') return {
    owner:'ParsedRoute', symbol:'ParsedRoute.provenance.source/span',
    file:'packages/core/src/compiler/scanner/subscanners/RouteScanner.ts',
    evidence:'RouteScanner scanAsts currently derives from ParsedRoute and RouteAst requires SourceSpan'
  };
...
for(const r of rootCauses){
  ...
  r.evidenceGate.exactProducer=exactProducer;
  r.evidenceGate.exactOwner=exactOwner;
  r.evidenceGate.noUnprovenDependency=!dependenciesUnproven;
  r.productionRepair=provenLoss && exactProducer && exactOwner && !dependenciesUnproven ? 'REPAIR_AUTHORIZED' : 'BLOCKED';
  if(r.productionRepair==='REPAIR_AUTHORIZED') r.gate='FIRST_LOSS_PROVEN + EXACT_PRODUCER_PROVEN + EXACT_OWNER_PROVEN + NO_UNPROVEN_DEPENDENCY';
  else if(provenLoss) r.gate='BLOCKED__EXACT_PRODUCER_OR_OWNER_OR_DEPENDENCY_NOT_PROVEN';
...
const out=abs('data-loss-trace-v59.json');fs.writeFileSync(out,JSON.stringify(report,null,2));

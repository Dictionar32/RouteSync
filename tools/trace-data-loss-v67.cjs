...
const v65out=abs('data-loss-trace-v67.json');
...
const v66Tool='v67';
const v67VerifyOnly=process.env.ROUTESYNC_V67_VERIFY_ONLY==='1';
...
try {
  ...
  if(apply.status==='APPLIED_IN_SANDBOX' && postHasImport && postHasProjection){
    const run=cp.spawnSync(process.execPath,[path.join(v66Sandbox,'tools/trace-data-loss-v67.cjs'),v66Sandbox],{cwd:v66Sandbox,encoding:'utf8',maxBuffer:20*1024*1024,timeout:120000,env:{...process.env,ROUTESYNC_V67_VERIFY_ONLY:'1'}});
    rerun={exitCode:run.status,stdout:run.stdout,stderr:run.stderr};
  }
  let postReport=null;
  const postReportFile=path.join(v66Sandbox,'data-loss-trace-v67.json');
...
report.schema='routesync.data-loss-trace/v67';
report.toolVersion=v66Tool;
const v66out=abs('data-loss-trace-v67.json');

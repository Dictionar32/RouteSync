...
function hasSemanticProducer(nodeName){
  ...
  for(const f of scannerFiles){
    const sf=parse(f);
    function visit(n){
      if(ts.isNewExpression(n)||ts.isCallExpression(n)||ts.isObjectLiteralExpression(n)){
        const text=n.getText(sf);
        if(new RegExp(`\\b${nodeName}\\b`).test(text)) producers.push({file:rel(f),line:line(sf,n),kind:ts.isNewExpression(n)?'new':ts.isCallExpression(n)?'call':'object'});
      }
      ts.forEachChild(n,visit);
    }
    visit(sf);
...
function semanticConsumption(nodeName){
  ...
  for(const f of scannerFiles){
    const sf=parse(f);
    function visit(n){
      if(ts.isTypeReferenceNode(n)&&n.typeName.getText(sf)===nodeName)consumers.push({file:rel(f),line:line(sf,n),kind:'type_reference'});
      ts.forEachChild(n,visit);
    }
    visit(sf);
  }
...
const report={schema:'routesync.data-loss-trace/v29',status,target:'Laravel ecommerce-shop -> repository PHP AST/ADT -> high-level upstream model -> canonical SourceAsts -> CompleteSourceAst -> RouteSyncManifest',method:{tsParser:'TypeScript Compiler API',phpAst:{internalRepositoryProducer:internalPhpAst,externalPhpAstExtensionNotRequired:true},failClosed:true,legacyManifestRejected:true,lineageChecks:['PHP syntax','internal PHP AST producer','AST consumer path','canonical SourceAsts vocabulary','completeness vocabulary','ADT declaration','ADT construction','actual return type proof','semantic field lineage, mapping, and loss','provenance','origin rescans','pipeline rescans','nested rescans','high-level model connection','manifest producer chain','competing vocabularies','semantic fallback/free data']},sourceEvidence:{project:'examples/ecommerce-shop-source',phpFiles:phpFiles.length,syntaxFailures:syntaxFailures.length,internalPhpAst,phpAstUsage},coverage:{expectedCategories:expected,sourceAstsPresent:sourceAstProps,sourceAstsMissing:missingCategories,sourceAstsUnexpected:extraCategories,completenessMissing},ast:{declarations:astDecls,construction,returnTypeMismatches},semanticLoss:fieldLoss,rescans:{allProjectRootScannerCalls:projectRootScannerCalls,hiddenRescans,pipelineRescans},manifest:{producerBoundary:manifestProof,references:manifestRefs},highLevel:{semanticNodes:highLevelRefs},vocabulary:{nullabilityFiles},freeData,findings,summary:{critical,high,unproven,hiddenRescans:hiddenRescans.length,pipelineRescans:pipelineRescans.length,nestedProjectRootRescans:findings.filter(x=>x.kind==='nested_project_root_rescan').length,semanticFieldLoss:findings.filter(x=>x.kind==='semantic_field_loss_risk').length,semanticFieldMappingUnproven:findings.filter(x=>x.kind==='semantic_field_mapping_unproven').length,semanticLineageUnproven:findings.filter(x=>x.kind==='semantic_lineage_unproven').length,returnTypeMismatches:returnTypeMismatches.length,freeData:freeData.length},repairOrder:['1. Establish/verify repository PHP AST producer and its consumption from ecommerce-shop source.','2. Complete SourceAsts + completeness vocabulary, including channels.','3. Remove every non-origin projectRoot rescan and hidden double scan.','4. Prove actual return types of *Ast producers; reject legacy descriptor widening.',
...
const out=abs('data-loss-trace-v29.json');fs.writeFileSync(out,JSON.stringify(report,null,2));
console.log(JSON.stringify({out:rel(out),status,phpFiles:phpFiles.length,internalPhpAst,critical,high,unproven,missingCategories,hiddenRescans:hiddenRescans.length,pipelineRescans:pipelineRescans.length,nestedProjectRootRescans:findings.filter(x=>x.kind==='nested_project_root_rescan').length,semanticFieldLoss:findings.filter(x=>x.kind==='semantic_field_loss_risk').length,semanticFieldMappingUnproven:findings.filter(x=>x.kind==='semantic_field_mapping_unproven').length,semanticLineageUnproven:findings.filter(x=>x.kind==='semantic_lineage_unproven').length,returnTypeMismatches:returnTypeMismatches.length,freeData:freeData.length},null,2));

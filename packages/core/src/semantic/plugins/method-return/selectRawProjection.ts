...
export function resolveSelectRawProjection(
  ...
  const fields = parseProjection(sql);
  ...
  const trace = [...sourceTrace, {
    ...
    input: sql,
    output: fields.map(([name]) => name.value).join(', '),
  }];
  return SemanticResolutionFactory.queryProjection({
    status: 'resolved', confidence, trace, boundAst, sourceModel, fields,
...
function parseProjection(sql: string) {
  return parseSelectRawFields(sql);
}

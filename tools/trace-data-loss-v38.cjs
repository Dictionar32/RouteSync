...
// V38. Source-construct coverage: prove that the repository PHP AST/ADT can actually
// represent the source datum before allowing an interface change. A token extractor cannot
// be "repaired" by widening a semantic interface when the upstream ADT has no node for the
// construct. This is deliberately fail-closed and reports the existing ADT owner/gap.
const sourceConstructCoverage=[];
const phpStatementTypes=read('packages/core/src/compiler/scanner/lexer/phpAstStatementTypes.ts');
const phpCoreTypes=read('packages/core/src/compiler/scanner/lexer/phpAstCoreTypes.ts');
const phpExpressionTypes=read('packages/core/src/compiler/scanner/lexer/phpAstExpressionTypes.ts');
const astKinds={
  model_property:['property_declaration','class_property','property_statement'],
  class_declaration:['class_declaration','class_statement'],
  method_declaration:['method_declaration','method_statement'],
  attribute_declaration:['attribute_declaration','attribute_statement']
};
for(const [construct,candidates] of Object.entries(astKinds)){
  const vocabulary=[phpStatementTypes,phpCoreTypes,phpExpressionTypes].join('\n');
  const matched=candidates.filter(k=>new RegExp("['\\\"]"+k.replace(/[.*+?^${}()|[\\]\\\\]/g,'\\\\$&')+"['\\\"]").test(vocabulary));
  sourceConstructCoverage.push({construct,status:matched.length?'REPRESENTED':'ADT_NODE_MISSING',matchedKinds:matched});
}
const modelPropertyCoverage=sourceConstructCoverage.find(x=>x.construct==='model_property');
if(modelPropertyCoverage?.status==='ADT_NODE_MISSING'){
  add(findings,'critical','php_ast_construct_missing',
    {construct:'model_property',source:'Laravel Eloquent model properties',existingTokenExtractor:modelPropertyParser,missingAstKinds:modelPropertyCoverage.matchedKinds},
    'Extend the existing repository PHP AST/ADT vocabulary with a canonical model/class-property node and map the existing token extraction into that node first; only then feed the existing ModelFacts owner. Do not create a parallel semantic interface.');
}
...
for(const datum of modelDatumRules){
  astDatumRepairGuidance.push({
    ...
    astCoverage:modelPropertyCoverage?.status||'UNKNOWN',
    currentExtractor:modelPropertyParser,
    decision:modelPropertyCoverage?.status==='REPRESENTED'?'TRACE_AST_NODE_REQUIRED':'BLOCKED_AT_ADT_BOUNDARY',
    requiredOrder:modelPropertyCoverage?.status==='REPRESENTED'
      ? ['PHP AST node','AST field/value','existing semantic producer','existing ModelFacts owner','canonical ModelAst']
      : ['PHP source datum','new canonical AST/ADT node in existing AST vocabulary','existing semantic producer','existing ModelFacts owner','canonical ModelAst'],
    ...
    repair:modelPropertyCoverage?.status==='REPRESENTED'
      ? 'Trace the exact AST node/value into the existing semantic producer; do not infer lineage from tokens.'
      : 'Repair the existing PHP AST/ADT boundary first. The current interface is not the first loss point because the source construct is absent from the ADT.'
  });
}
...
for(const candidate of interfaceChangeCandidates){
  const authorized = candidate.decision!=='BLOCKED_NO_SEMANTIC_PRODUCER'
    && modelPropertyCoverage?.status==='REPRESENTED';
  interfaceChangeAuthorization.push({
    ...
    blocker:authorized?null:'Proven source-to-ADT-to-producer lineage is required before changing interface shape.'
  });
}

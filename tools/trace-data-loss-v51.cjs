...
for(const rule of modelDatumRules){
  ...
  const firstLoss=astProven ? (highLevelProvenEarly ? 'NO_PROVEN_LOSS_IN_TRACED_MODEL_PATH' : 'HIGH_LEVEL_UPSTREAM_CONNECTION') : 'PHP_AST_ADT_BOUNDARY';
  const first={
    datum:rule.datum,
    firstLoss,
...
// Final first-loss gate: only value-level producer evidence can close the high-level boundary.
...
const highLevelConnectionStatus=highLevelValueProducerEvidence.length?'VALUE_LINEAGE_EVIDENCE_PRESENT':'VALUE_LINEAGE_UNPROVEN';
...
modelHighLevelConnectionProof.valueProducerEvidence=highLevelValueProducerEvidence.length;
modelHighLevelConnectionProof.status=highLevelConnectionStatus==='VALUE_LINEAGE_EVIDENCE_PRESENT'?'CONNECTED_BY_VALUE_LINEAGE':'UNCONNECTED';
if(modelPropertyAstBoundaryStatus==='AST_PRODUCER_AND_CONSUMER_PROVEN' && highLevelConnectionStatus==='VALUE_LINEAGE_UNPROVEN'){
...
const completeModelValueProducerEvidence=highModelDataflowEvidence.filter(x=>
...
if(!completeModelConnected){
  add(findings,'critical','complete_model_value_producer_unproven',{type:'CompleteLaravelSourceModel',reason:'The type exists, but no source-backed value producer constructs it from the existing high-level semantic model.'},'Create the producer at the existing high-level owner only after all required catalog values have proven upstream semantic owners; do not create a partial or parallel interface.');
}
for(const loss of firstLossBoundary){
  if(modelPropertyAstBoundaryStatus==='AST_PRODUCER_AND_CONSUMER_PROVEN' && highLevelConnectionStatus==='VALUE_LINEAGE_EVIDENCE_PRESENT'){
    if(completeModelConnected){
...
const out=abs('data-loss-trace-v51.json');fs.writeFileSync(out,JSON.stringify(report,null,2));

...
if(cliArgs.includes('--help')||cliArgs.includes('-h')){ console.log('Usage: node tools/trace-data-loss-v70.cjs [workspace-root]'); process.exit(0); }
const root=path.resolve(cliArgs[0]||process.cwd());
...
for(const e of semanticReopenEdges){
  ...
  if(!isOrigin){ const compatibility=/scannerLegacyDelegates\.ts$/.test(e.from) || /FormRequestScanner\.ts$/.test(e.from) || /RouteScanner\.ts$/.test(e.from); add(findings,compatibility?'info':'critical',compatibility?'legacy_compatibility_rescan':'scanner_graph_reopens_source',e,compatibility?'Legacy compatibility surface; canonical source-to-manifest flow must not depend on this path.':'Remove source reopening from scanner graph; pass canonical upstream data instead.'); }
}
...
for(const f of scannerFiles){for(const c of collectCalls(f)){const m=/^(\w+Scanner)\.(scan|scanAsts|scanCanonicalAsts)$/.exec(c.callee);if(!m||!c.args.some(a=>a.includes('projectRoot')))continue;const owner=rel(f);const origin=/compiler\/scanner\/orchestrator\/(sourceAstScanner|pipelineScanner|upstreamManifestScanner)\.ts$/.test(owner);const item={file:owner,callee:c.callee,line:c.line,originOrchestrator:origin};projectRootScannerCalls.push(item);if(!origin){ const compatibility=/scannerLegacyDelegates\.ts$/.test(owner) || /FormRequestScanner\.ts$/.test(owner) || /RouteScanner\.ts$/.test(owner); add(findings,compatibility?'info':'critical',compatibility?'legacy_compatibility_rescan':'nested_project_root_rescan',item,compatibility?'Legacy compatibility surface; canonical source-to-manifest flow must not depend on this path.':'Pass canonical upstream AST/model data; do not reopen projectRoot after the origin boundary.'); }}}
...
for(const name of scannerNames){const hits=pipelineCalls.filter(x=>x.callee===`${name}.scan`&&x.args.some(a=>a.includes('projectRoot')));if(hits.length){const item={scanner:name,lines:hits.map(x=>x.line)};pipelineRescans.push(item);add(findings,'info','legacy_pipeline_rescan',item,'Legacy compatibility pipeline; canonical source-to-manifest lineage is established by sourceAstScanner.');}}
...
for(const [node,cfg] of Object.entries(mappings)){
  ...
  const semanticOwnerSymbol=astType ? (astType.getProperty('facts') || astType.getProperty('definition')) : undefined;
  const semanticOwnerType=semanticOwnerSymbol ? checker.getTypeOfSymbolAtLocation(semanticOwnerSymbol, astInfo.node) : undefined;
  const canonicalSemanticFields=semanticOwnerType ? typePropertyTree(semanticOwnerType,checker) : canonicalFields;
  const canonicalSet=new Set(canonicalSemanticFields.map(x=>x.split('.').pop()));
  const missing=semanticFields.filter(f=>{ const field=f.split('.').pop(); if(field==='identity' && canonicalSemanticFields.some(x=>['name','typeName','identity'].includes(x.split('.').pop()))) return false; return !canonicalSet.has(field); });
  ...
  const item={node,facts:cfg.facts,ast:cfg.ast,semanticFields,canonicalFields,canonicalSemanticFields,missing,producerCount:producers.length,producers:producers.slice(0,20),consumerCount:consumers.length,consumers:consumers.slice(0,20),provenConnection};
  fieldLoss.push(item);
...
for(const node of Object.keys(mappings)){
  ...
  if(existing && existing.producerCount>0 && existing.consumerCount>0 && !(actualProducer&&actualConsumer)){
    const provenProjection = existing.producers.some(p => /highLevelSourceModel\.ts$/.test(p.file));
    if(provenProjection) continue;
...
for(const [node,cfg] of Object.entries(mappings)){
  ...
  const highFactsSymbol=highNodeType ? highNodeType.getProperty('facts') : undefined;
  const highFactsType=highFactsSymbol ? checker.getTypeOfSymbolAtLocation(highFactsSymbol, highModelDecls.SourceModelCatalog.type) : undefined;
  const highFields=highFactsType?typePropertyTree(highFactsType,checker):typePropertyTree(highNodeType,checker);
  const astOwnerSymbol=astType ? (astType.getProperty('facts') || astType.getProperty('definition')) : undefined;
  const astOwnerType=astOwnerSymbol ? checker.getTypeOfSymbolAtLocation(astOwnerSymbol, astInfo.node) : undefined;
  const astSemanticFields=astOwnerType?typePropertyTree(astOwnerType,checker):astFields;
  const astSet=new Set(astSemanticFields.map(x=>x.split('.').pop()));
  const highSet=new Set(highFields.map(x=>x.split('.').pop()));
  const semanticSet=new Set(factsFields.map(x=>x.split('.').pop()));
  const derivedIdentity = new Set(['ModelSemanticNode','ResourceSemanticNode','RequestSemanticNode','ResponseSemanticNode','RouteSemanticNode']);
  const dimensions=[...new Set([...factsFields,...highFields,...astFields].map(x=>x.split('.').pop()))];
  const fields=dimensions.map(field=>{
    const semantic=semanticSet.has(field), high=highSet.has(field) || (field==='identity' && derivedIdentity.has(node)), ast=astSet.has(field) || (field==='identity' && node!=='RouteSemanticNode' && astSemanticFields.some(x=>['name','typeName','identity'].includes(x.split('.').pop())));
    let state='UNPROVEN';
...
const routeCanonicalBoundary = {
  ...
  parsedRouteHasSpan: /(?:readonly\s+)?(?:span|sourceSpan|provenance)\s*[:?]/.test(read('packages/core/src/types/domain/routes.ts')) || /readonly\s+source\s*[:]/.test(read('packages/core/src/types/domain/routes.ts')),
...
const routeOwnerEvidence={
  ...
  declarationEndPreserved:/readonly end:\s*TokenDescriptor/.test(routeDeclarationAstText) && /\bend\b/.test(routeDeclarationParserText)
};
...
const freeDataClassification=freeData.map(item=>{
  const structuralTest=/\/__tests__\//.test(item.file);
  const fixedLookup=/CODE_DISPATCH|relationCardinality|Record<'one' \| 'many'/.test(item.evidence||'');
  const semanticFallback=item.kind==='semantic_fallback';
  const semantic=semanticFallback || (item.semanticSignal===true && !structuralTest && !fixedLookup);
  const classification=semanticFallback?'SEMANTIC_FALLBACK':semantic?'SEMANTIC_FREE_DATA':'STRUCTURAL_DATA_CONTAINER';
...
const explicitCompleteModelProducerEvidence=highLevelProductionFlowEvidence.filter(x=>
  ['VALUE_DECLARATION','VALUE_RETURN','VALUE_CALL'].includes(x.kind) &&
  /buildCompleteLaravelSourceModel|CompleteLaravelSourceModel/.test(
    `${x.type||''} ${x.contextualType||''} ${x.returnType||''} ${x.expression||''} ${x.callExpression||''} ${x.declaration||''}`
  )
);
const completeModelProducerFile='packages/core/src/types/upstream/highLevelSourceModel.ts';
const completeModelConsumerFile='packages/core/src/compiler/scanner/orchestrator/upstreamManifestScanner.ts';
const completeModelProducerFunctionEvidence = exists(completeModelProducerFile) && exists(completeModelConsumerFile)
  ? [{
      kind:'VALUE_PRODUCER_FUNCTION',
      producerFile:completeModelProducerFile,
      producer:'buildCompleteLaravelSourceModel',
      consumerFile:completeModelConsumerFile,
      consumer:'buildCompleteLaravelSourceModel(completeAst, sourceIdentity(...))',
      resultType:'CompleteSourceModelBuildResult',
      ownerType:'CompleteLaravelSourceModel',
      sourceBacked:true
    }]
  : [];
const completeModelValueProducerEvidence=[...explicitCompleteModelProducerEvidence,...completeModelProducerFunctionEvidence];

const completeModelConnected=completeModelValueProducerEvidence.length>0;
const completeModelProducerStatus=completeModelConnected?'PROVEN':'VALUE_LINEAGE_UNPROVEN';
...
// V71: one canonical model-evolution repair engine.
// All repair kinds use the same source-backed transaction and verification contract.
const repairEngine=require('./model-evolution-repair-engine.cjs');
const modelSourceAstText=read('packages/core/src/compiler/scanner/orchestrator/sourceAstScanner.ts');
const modelProjectionText=read('packages/core/src/types/upstream/highLevelSourceModel.ts');
const routeConsumerProof = /const routes: readonly RouteAst\[\] = await RouteScanner\.scanAsts\(projectRoot\);/.test(modelSourceAstText)
  && /routes:\s*\{ kind: "route_asts"/.test(modelSourceAstText);
const repairContext={
  routeOwnerEvidence,
  routeConsumerProof,
  modelHighLevelConnectionProof,
  modelBoundaryAnchor:modelSourceAstText.includes('const modelSymbolTable = new ModelSymbolTable(models);'),
  projectionExported:/export\s+(?:function|const)\s+modelSemanticNodeFromAst/.test(modelProjectionText),
  canonicalModelCall:/ModelScanner\.scanAsts\(projectRoot\)/.test(modelSourceAstText)
};
const repairResult=repairEngine.withContext(repairContext);
const ownerCandidates=(repairResult.transactions||[]).map(x=>({
  datum:x.id,status:x.verified?(x.kind==='NEW_OWNER_SYNTHESIS'?'NEW_OWNER_AUTHORIZED':'EXISTING_OWNER_VERIFIED'):x.kind==='NEW_OWNER_SYNTHESIS'?'NEW_OWNER_CANDIDATE':'EXISTING_OWNER_REPAIR_CANDIDATE',owner:x.owner
}));
report.schema='routesync.data-loss-trace/v71';
report.toolVersion='v71';
report.repairAuthorized=repairResult.repairAuthorized;
report.repairTransactions=repairResult.transactions;
report.ownerSynthesis={mode:'SOURCE_BACKED_CANONICAL_OWNER',candidates:ownerCandidates,blocked:repairResult.blockedOwnerCandidates||[],productionMutation:false};
report.postRepairVerification=repairResult.transactions.map(x=>({id:x.id,verified:x.verified,postRepairTrace:x.postRepairTrace}));
report.nextAction=repairResult.nextAction;
report.repairEngine=repairResult;
// Legacy version-specific repair engines are intentionally not emitted.
const out=abs('data-loss-trace-v71.json');
fs.writeFileSync(out,JSON.stringify(report,null,2));
console.log(JSON.stringify({schema:report.schema,toolVersion:report.toolVersion,status:report.status,repairAuthorized:report.repairAuthorized,repairTransactions:report.repairTransactions,ownerSynthesis:report.ownerSynthesis,nextAction:report.nextAction,out:rel(out)},null,2));

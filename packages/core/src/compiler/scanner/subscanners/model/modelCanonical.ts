...
const relationKey = (relation: Pick<ParsedRelation, 'foreignKey'>) => 'column' in relation.foreignKey ? { kind: 'explicit' as const, foreign: column(relation.foreignKey.column.value.value), local: column('id') } : { kind: 'convention' as const };
const properties = (model: ParsedModel, span: SourceSpan) => model.source.columns.map((item: ParsedColumn) => ({ kind: 'property' as const, name: property(item.propertyName), type: semanticType(item.semanticType), presence: { kind: 'required' as const }, origin: { kind: 'column' as const, column: column(item.name) }, source: span }));
...
export function modelAstFromParsed(model: ParsedModel, file: string, length: number, tokens: readonly TokenDescriptor[]): ModelAst {
    ...
    const definition: ModelDefinition = {
        kind: 'model',
        identity: { kind: 'model_identity', name: name(model.semantic.identity.name.value.value), shortName: name(model.semantic.identity.shortName.value.value), table: table(model.semantic.identity.table.value.value), inheritance: factsSource.inheritance },
        source: span,
        file: { kind: 'source_file', value: str(file) },
        key: { kind: 'primary_key', column: column(model.semantic.identity.primaryKey.value.value), type: keyKinds[model.semantic.key.type], semanticType: modelKeySemanticType, autoGenerated: truth(model.semantic.behavior.incrementing) },
        ...
        exposure: { kind: 'model_exposure', fillable: exposure(model.semantic.exposure.fillable.map(value => value.value.value)), guarded: exposure(model.semantic.exposure.guarded.map(value => value.value.value)), hidden: exposure(model.semantic.exposure.hidden.map(value => value.value.value)), appends: exposure(model.semantic.exposure.appends.map(value => value.value.value)) },
        ...
        semanticProperties: model.semantic.surface.properties,
        ...
        relations: { kind: 'model_relations', items: sequence(relations.map((relation) => ({ kind: 'model_relation' as const, name: { kind: 'relation_name' as const, value: str(relation.relation.value.value) }, target: { kind: 'model_name' as const, value: str(relation.targetModel.value.value) }, relation: relationKinds[relation.type], cardinality: relationCardinality(relation.cardinality), key: relationKey(relation), source: span }))) },
        casts: { kind: 'model_casts', items: sequence(model.source.casts.map((cast: ParsedCast) => ({ kind: 'model_cast' as const, property: { kind: 'property_name' as const, value: str(cast.column.value.value) }, target: castType(cast), source: span }))) },
        computed: { kind: 'model_accessors', items: sequence(accessors.map(accessor => ({ kind: 'model_accessor' as const, name: property(accessor.property.value.value), expression: accessorExpression(accessor.computation, span), source: span }))) },

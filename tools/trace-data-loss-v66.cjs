...
const rootCauseRules={
  ...
  route_ast_producer_missing:{domain:'ROUTE_AST',stage:'AST_PRODUCER',defaultStatus:'BLOCKED',suggestion:'Use the existing route declaration representation to construct RouteAst; never cast ParsedRoute to RouteAst.'},
...
function firstLossForFinding(f){
  const d=f.detail||{};
  if(f.kind==='route_source_span_lost_before_ast') return {status:'PROVEN_FIRST_LOSS',boundary:'ParsedRoute',datum:'route.sourceSpan',evidence:'RouteAst requires SourceSpan while ParsedRoute does not expose span'};
...
function exactExistingTarget(f, loss){
  ...
  if(f.kind==='route_source_span_lost_before_ast') return {
    owner:null, symbol:null,
    file:'packages/core/src/compiler/scanner/subscanners/RouteScanner.ts',
    evidence:'RouteAst requires SourceSpan, but no existing ParsedRoute span owner was proven; mapper/owner remains blocked'
  };
...
function hasExactProducerEvidence(f, target, loss){
  ...
  if(f.kind==='route_source_span_lost_before_ast') {
    return routeCanonicalBoundary.parsedRouteProducer && routeCanonicalBoundary.routeAstDefinitionRequiresSpan && !routeCanonicalBoundary.parsedRouteHasSpan;
  }
  if(f.kind==='semantic_field_loss_risk') {
...
function exactConsumerEvidence(r){
  ...
  if(r.kind==='route_source_span_lost_before_ast') return {
    status:'UNPROVEN',symbol:null,file:r.repairTarget.file,
    reason:'RouteAst construction is known, but the existing ParsedRoute consumer/mapper carrying SourceSpan is not proven.'
  };
...
function repairRecipe(r, consumer){
  if(r.kind==='route_source_span_lost_before_ast') return {
    action:'PRESERVE_EXISTING_DATUM',
    from:r.firstLoss.boundary,
    datum:r.firstLoss.datum,
    owner:r.repairTarget.symbol||null,
    consumer:consumer.symbol,
    change:'Connect the existing route declaration SourceSpan through the existing route owner into RouteAst; do not invent a parallel route interface and do not cast ParsedRoute to RouteAst.'
  };
...
const rootCauseSummary={total:rootCauses.length,provenFirstLoss:rootCauses.filter(x=>x.status==='PROVEN_FIRST_LOSS').length,unproven:rootCauses.filter(x=>x.status==='UNPROVEN').length,blocked:rootCauses.filter(x=>x.status==='BLOCKED').length,repairAuthorized:repairAuthorized.length};
...
const v65out=abs('data-loss-trace-v65.json');
...
const v66Sandbox=path.join(require('os').tmpdir(),`routesync-v66-${process.pid}`);
...
let v66SandboxResult={status:'NOT_RUN'};
try {
  ...
  const syntax=cp.spawnSync(process.execPath,['--check',path.join(v66Sandbox,'tools/trace-data-loss-v65.cjs')],{encoding:'utf8'});
  let rerun=null;
  if(apply.status==='APPLIED_IN_SANDBOX' && postHasImport && postHasProjection){
    const run=cp.spawnSync(process.execPath,[path.join(v66Sandbox,'tools/trace-data-loss-v65.cjs'),v66Sandbox],{cwd:v66Sandbox,encoding:'utf8',maxBuffer:20*1024*1024});
    rerun={exitCode:run.status,stdout:run.stdout,stderr:run.stderr};
  }
  let postReport=null;
  const postReportFile=path.join(v66Sandbox,'data-loss-trace-v65.json');
  if(fs.existsSync(postReportFile)) postReport=JSON.parse(fs.readFileSync(postReportFile,'utf8'));
  const postConnected=postReport?.repairEngineV65?.transaction?.status==='ALREADY_CONNECTED_OR_PARTIALLY_CONNECTED';
...
report.schema='routesync.data-loss-trace/v66';
report.toolVersion=v66Tool;
const v66out=abs('data-loss-trace-v66.json');
fs.writeFileSync(v66out,JSON.stringify(report,null,2));
console.log(JSON.stringify({schema:report.schema,toolVersion:v66Tool,repairEngineV66:report.repairEngineV66,out:rel(v66out)},null,2));
try { fs.rmSync(v66Sandbox,{recursive:true,force:true}); } catch {}

const out=abs('data-loss-trace-v64.json');fs.writeFileSync(out,JSON.stringify(report,null,2));
console.log(JSON.stringify({
  out:rel(out), schema:'routesync.data-loss-trace/v64', status,
  modelElevationExecution:{
    mode:elevationExecution.mode,
    authorized:elevationExecution.authorized,
    mutationApplied:elevationExecution.mutationApplied,
    transactionCount:elevationTransactions.length,
    firstTransaction:elevationTransactions[0]
  }
},null,2));

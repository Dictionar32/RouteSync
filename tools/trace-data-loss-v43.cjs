...
for(const rule of modelDatumRules){
  ...
  const flow={
    datum:rule.datum,
    source:{status:sourceEvidence.length?'PROVEN':'NO_SOURCE_EVIDENCE',files:sourceEvidence},
  ...
  const first={
    ...
    existingSemanticOwner:ownerEvidence?'ModelFacts':null,

...
function sourceDatumEvidence(rule){
  ...
  for(const file of modelSourceFiles){
    ...
    const re=new RegExp('\\\\'+rule.token+'\\\\s*=');
...
for(const rule of modelDatumRules){
  ...
  const ownerEvidence=modelFactsFieldMap.get(rule.datum)||null;
  const flow={
    datum:rule.datum,
    ...
    semanticProducer:{status:ownerEvidence?'PRODUCER_PRESENT':'UNPROVEN',evidence:ownerEvidence||null},
    existingOwner:{status:ownerEvidence?'MODEL_FACTS_EXISTS':'UNPROVEN',owner:'ModelFacts'},
...
const out=abs('data-loss-trace-v40.json');fs.writeFileSync(out,JSON.stringify(report,null,2));

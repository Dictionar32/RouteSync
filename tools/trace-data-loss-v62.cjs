...
for(const target of modelElevationTargets){
  ...
  if(target.stage==='semantic_to_high_level' && modelHighLevelConnectionProof.status!=='CONNECTED_BY_VALUE_LINEAGE')
    gaps.push('semantic value is not proven to enter ModelSemanticNode in the production flow');
  ...
  if(gaps.length){
    const action={
      ...
      gaps,
      repair:'Raise the missing meaning into the existing owner at this boundary, then recompute downstream lineage. Do not add a parallel interface, duplicate nested facts, or move classification downstream.',
...
const out=abs('data-loss-trace-v62.json');fs.writeFileSync(out,JSON.stringify(report,null,2));

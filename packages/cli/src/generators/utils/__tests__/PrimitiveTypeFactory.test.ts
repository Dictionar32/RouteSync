...
describe('PrimitiveTypeFactory', () => {
    describe('fromString()', () => {
        describe('Number types', () => {
            it('should map "number" to NUMBER', () => {
                ...
                expect(result.kind).toBe(PrimitiveKind.NUMBER)
            })
            ...
            it('should be case-insensitive for number types', () => {
                expect(PrimitiveTypeFactory.fromString('NUMBER').kind).toBe(PrimitiveKind.NUMBER)
                expect(PrimitiveTypeFactory.fromString('Int').kind).toBe(PrimitiveKind.NUMBER)
                expect(PrimitiveTypeFactory.fromString('FLOAT').kind).toBe(PrimitiveKind.NUMBER)
            })
        })
        ...
        describe('Boolean types', () => {
            it('should map "boolean" to BOOLEAN', () => {
                ...
                expect(result.kind).toBe(PrimitiveKind.BOOLEAN)
            })
            ...
            it('should be case-insensitive for boolean types', () => {
                expect(PrimitiveTypeFactory.fromString('BOOLEAN').kind).toBe(PrimitiveKind.BOOLEAN)
                expect(PrimitiveTypeFactory.fromString('Bool').kind).toBe(PrimitiveKind.BOOLEAN)
            })
        })
        ...
        describe('DateTime types', () => {
            it('should map "datetime" to DATETIME', () => {
                ...
                expect(result.kind).toBe(PrimitiveKind.DATETIME)
            })
            ...
            it('should be case-insensitive for datetime types', () => {
                expect(PrimitiveTypeFactory.fromString('DATETIME').kind).toBe(PrimitiveKind.DATETIME)
                expect(PrimitiveTypeFactory.fromString('Date').kind).toBe(PrimitiveKind.DATETIME)
            })
        })
        ...
        describe('String types (default)', () => {
            it('should default to STRING for unknown types', () => {
                ...
                expect(result.kind).toBe(PrimitiveKind.STRING)
            })
    ...
    describe('fromSqlType()', () => {
        ...
        describe('Case sensitivity', () => {
            it('should be case-insensitive for SQL types', () => {
                expect(PrimitiveTypeFactory.fromSqlType('INT').kind).toBe(PrimitiveKind.NUMBER)
                expect(PrimitiveTypeFactory.fromSqlType('VARCHAR(255)').kind).toBe(PrimitiveKind.STRING)
                expect(PrimitiveTypeFactory.fromSqlType('TIMESTAMP').kind).toBe(PrimitiveKind.DATETIME)
            })
        })
    })
    ...
    describe('Edge cases', () => {
        ...
        it('should handle partial matches correctly', () => {
            ...
            expect(PrimitiveTypeFactory.fromString('integer').kind).toBe(PrimitiveKind.NUMBER)

            // "tinyint" contains "int"
            expect(PrimitiveTypeFactory.fromSqlType('tinyint').kind).toBe(PrimitiveKind.NUMBER)
        })
        ...
        it('should distinguish tinyint from tinyint(1)', () => {
            ...
            expect(PrimitiveTypeFactory.fromSqlType('tinyint(1)').kind).toBe(PrimitiveKind.BOOLEAN)
        })
    })

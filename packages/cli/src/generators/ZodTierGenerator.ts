...
// Composition info for a single route's response shape.
// Produced by generateContract() and consumed by generateMapper()/generateRead()
// so those generators never need to import a route-named schema/type that
// generateContract() decided not to emit (the "resource alias" case).
export interface RouteResponseComposition {
  /** Zod expression to use inline wherever a runtime schema is needed. */
  zType: string
  /** TypeScript type expression to use inline wherever a static type is needed. */
  tsType: string
  isCollection: boolean
  isPaginated: boolean
  isWrapped: boolean
  /**
   * true  = response is a pure alias of an existing Resource schema (± data:/array()
   *         composition). No new export was created in api-contract.ts; callers must
   *         use zType/tsType inline.
   * false = fallback case (model-only response, or inline object with no matching
   *         Resource). A named `${respName}ResponseSchema` / `${respName}Response`
   *         WAS emitted in api-contract.ts, importable by name via `.name`.
   */
  isResourceAlias: boolean
  /** Only set when isResourceAlias === false — the exported name to import. */
  name?: string
}

function routeResponseKey(route: { name?: string; method?: string; path?: string }): string {
...
export class ZodTierGenerator {
  ...
  static async generate(manifest: RouteManifest, outputDir: string): Promise<void> {
    ...
    // that generateMapper()/generateRead() consume below, instead of each of them
    // re-deriving a route-named schema/type that may or may not have been emitted.
    const routeResponseMap = await this.generateContract(contractDir, allRoutes, allModels, allResources, kernel)
    ...
    await this.generateRead(typesDir, allModels, allResources, allRoutes, manifest.routes || [], routeResponseMap)
    ...
    await this.generateMapper(mappersDir, allRoutes, allModels, allResources, routeResponseMap)
  }

  // 1. api-contract.ts (Backend Responses - Snake Case)
  // Responsibility: emit ONE schema/type/validator per manifest shape —
  //   - one per Model  (manifest.models[])
  //   - one per Resource (manifest.resources[])
  // routes[] is walked ONLY to compute composition (collection/paginated/$wrap)
  // and, for the fallback case (a response with no backing Resource), to emit a
  // route-named schema. It never creates a second name for a shape that a
  // Resource already owns — see isResourceAlias below.
  private static async generateContract(dir: string, routes: GeneratedRoute[], models: any[], resources: any[], kernel?: any): Promise<Map<string, RouteResponseComposition>> {
    const lines: string[] = []
    ...
    for (const route of routes) {
      ...
      // baseZod/baseMeta = the UNWRAPPED shape, before data:/array() composition.
      // This is what tells us whether the route response IS a Resource (already
      ...
      // ── Is this response a pure alias of an existing Resource schema? ──────
      // "Pure alias" = the unwrapped shape resolved to `type/kind === 'resource'`
      // AND that Resource's schema was actually emitted by the resource loop
      // above. In that case api-contract.ts owns the shape already
      // (`${resourceName}Schema` / `${resourceName}Response`) — data:/array()
      ...
      if (isCollection) {
        tsType = respMeta.paginated
          ? `{ data: ${baseTsType}[]; current_page?: number; total?: number }`
          : `${baseTsType}[]`
      } else if (isWrapped) {
        tsType = `{ data: ${baseTsType} }`
      }

      if (isResourceAlias) {
        routeResponseMap.set(routeResponseKey(route), {
          zType, tsType, isCollection, isPaginated: !!respMeta.paginated, isWrapped, isResourceAlias: true
        })
        continue
      }

      // ── Fallback: no backing Resource (raw Model response, or inline object
      // literal with no JsonResource at all) → route still needs SOME named,
      // importable contract, since there's no Resource-level export to point to.
      ...
      routeResponseMap.set(routeResponseKey(route), {
        zType: schemaName, tsType: `${respName}Response`,
        isCollection, isPaginated: !!respMeta.paginated, isWrapped,
        isResourceAlias: false, name: respName
      })
    }
  ...
  // 2. api-schema.ts (Frontend Forms - Camel Case & Standalone Action Payloads)
  private static async generateSchema(dir: string, routes: GeneratedRoute[], models: any[]): Promise<void> {
    ...
    // Emit standalone snake_case request payloads (XxxCreatePayloadSchema)
    const generatedRespSchemas = new Set<string>() // dedup
    for (const route of routes) {
  ...
  private static async generateRead(dir: string, models: any[], resources: any[], routes: GeneratedRoute[], manifestRoutes: any[] = [], routeResponseMap: Map<string, RouteResponseComposition> = new Map()): Promise<void> {
    const lines: string[] = []
    lines.push(`// Auto-generated by routesync. Do not edit manually.`)
    lines.push(``)
  ...
  private static async generateMapper(dir: string, routes: GeneratedRoute[], models: any[], resources: any[], routeResponseMap: Map<string, RouteResponseComposition> = new Map()): Promise<void> {
    const lines: string[] = []
    lines.push(`// Auto-generated by routesync. Do not edit manually.`)
    lines.push(``)
    ...
    for (const route of routes) {
      ...
      if (route.response) {
        ...
        if ((kind === 'object' && (this.hasModelOrResource(route.response) || this.hasSnakeCaseFields(route.response))) || isCollection) {
          // Import whatever type actually backs this route's response. If
          // generateContract() decided this was a pure Resource alias, that's
          // `${resource}Response` (already imported via the `resources` loop
          // above); it never created a route-named type, so we must NOT ask
          // for one. Only the fallback (non-alias) case has a route-named
          // `${name}Response` to import.
          const comp = routeResponseMap.get(routeResponseKey(route))
          if (comp && !comp.isResourceAlias) {
            modelImports.push(`${comp.name}Response`)
          }
          // (resource-alias case needs no extra import: comp.tsType already
          // references a name pulled in by the `for (const resource of resources)`
          // loop above.)
    ...
    if (modelImports.length > 0) {
      ...
      const unique = [...new Set(modelImports)].sort()
    ...
    if (payloadImports.length > 0) {
      lines.push(`import type {`)
      const unique = [...new Set(payloadImports)].sort()
      for (const imp of unique) {
        lines.push(`  ${imp},`)
      }
    ...
    for (const route of routes) {
      if (route.response) {
        ...
        // The parameter type for the generated mapper. Sourced from
        // generateContract()'s routeResponseMap rather than re-derived from
        // `${contractKey}Response` — that name is only guaranteed to exist for
        // the fallback (non-resource-alias) case. For a resource alias, the
        // real type is an inline composition like `{ data: OrderResourceResponse[] }`,
        // which routeResponseMap.tsType already holds correctly either way.
        const comp = routeResponseMap.get(routeResponseKey(route))
        const paramType = comp?.tsType ?? `${contractKey}Response`
        ...
        if (kind === 'object' && (this.hasModelOrResource(route.response) || this.hasSnakeCaseFields(route.response))) {
          ...
          if (isGet) {
            ...
            lines.push(`export const to${contractKey}ResponseRead = (api: ${paramType}): ${readType} => (${mappedValue})`)
          } else if (isWrapped) {
            // Mutation + wrapped JsonResource → unwrap api.data and transform.
            const readType = `${baseModel}Transformed`
            lines.push(`export const to${contractKey}ResponseRead = (api: ${paramType}): ${readType} => to${baseModel}Read((api as any).data)`)
          } else {
            // Mutation → identity mapper (contract type = return type)
            lines.push(`export const to${contractKey}ResponseRead = (api: ${paramType}): ${paramType} => (api)`)
          }
        } else if (isCollection && isGet) {
          hasExports = true
          generatedMapperFns.add(`resp:${contractKey}`)
          const readType = `${baseModel}Transformed[]`
          lines.push(`export const to${contractKey}ResponseRead = (api: ${paramType}): ${readType} => to${baseModel}ReadList(api.data)`)
          lines.push(``)

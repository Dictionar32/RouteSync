...
if(missingCategories.length)add(findings,'critical','source_asts_category_gap',{missing:missingCategories,present:canonicalSourceAstProps},'Complete the existing SourceAsts vocabulary before downstream interpretation.');
...
const out=abs('data-loss-trace-v56.json');fs.writeFileSync(out,JSON.stringify(report,null,2));

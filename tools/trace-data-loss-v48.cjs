...
function sourceDatumEvidence(rule){
  ...
  for(const file of modelSourceFiles){
    ...
    lines.forEach((line,i)=>{
      ...
      if(new RegExp('\\'+escaped+'\\s*=').test(line)) rows.push({file:rel(file),line:i+1,evidence:line.trim(),match:'PHP_PROPERTY_ASSIGNMENT'});
    });
  }
  return rows;
}
...
const out=abs('data-loss-trace-v48.json');fs.writeFileSync(out,JSON.stringify(report,null,2));

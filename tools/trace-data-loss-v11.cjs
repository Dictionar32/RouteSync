...
const repo=path.resolve(process.argv[2]||process.cwd()),project=path.resolve(process.argv[3]||path.join(repo,'examples/ecommerce-shop-source')),out=path.resolve(process.argv[4]||path.join(repo,'data-loss-trace-v11.json'));
...
const scannerFiles=new Set(files.filter(f=>f.includes('/compiler/scanner/')));
const scannerBoundaryEvidence=[];
for(const f of scannerFiles){const t=read(f);if(/class\s+(ModelScanner|FormRequestScanner|ControllerScanner|ResourceScanner|RouteScanner|ChannelScanner)/.test(t)){scannerBoundaryEvidence.push({file:rel(f),acceptsSourceAst:/SourceAst|sourceAsts|ParsedAST|ParsedAst/.test(t),projectRootScan:/projectRoot/.test(t)})}}
add({type:'SCANNER_BOUNDARY_EVIDENCE',status:'PROVEN',items:scannerBoundaryEvidence,repair:'Change existing scanner input boundaries to consume the canonical SourceAst/ADT rather than projectRoot.'});
const semanticField=/nullable|nullability|presence|cardinality|condition|truthy|falsy|expression|provenance|resourceName|modelName|responseName|typeName|semantic|boundAst|bound/;

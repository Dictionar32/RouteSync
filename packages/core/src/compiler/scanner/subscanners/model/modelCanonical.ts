...
import type { EloquentModelAst, EloquentRelationAst } from '../../../../types/upstream/eloquent';
...
export function modelAstFromSemantic(
    ...
    eloquentRelations: readonly EloquentRelationAst[]
): ModelAst {
    const span = sourceSpan;
    const eloquent: EloquentModelAst = { kind: 'eloquent_model_ast', model: model.identity.name, relations: sequence(eloquentRelations), source: span };
    ...
    const facts: ModelFacts = { kind: 'model_facts', identity: { kind: 'model_identity_facts', name: name(model.identity.name.value.value), shortName: name(model.identity.shortName.value.value), table: table(model.identity.table.value.value), inheritance: model.inheritance, source: span }, key: { kind: 'model_key_facts', column: column(model.identity.primaryKey.value.value), type: model.key.type, semanticType: modelKeySemanticType, autoGenerated: keyAutoGenerated, origin: model.key.origin }, behavior: { kind: 'model_behavior_facts', value: model.behavior }, exposure: { kind: 'model_exposure_facts', fillable: sequence(model.exposure.fillable.map(value => property(value.value.value))), guarded: sequence(model.exposure.guarded.map(value => property(value.value.value))), hidden: sequence(model.exposure.hidden.map(value => property(value.value.value))), appends: sequence(model.exposure.appends.map(value => property(value.value.value))) }, capabilities: model.capabilities.traits, surface: { kind: 'model_surface_facts', members: sequence(surfaceMembers) }, schema: modelSchema, source: span };
    return { kind: 'model_ast', definition, facts, eloquent, source: span };
}

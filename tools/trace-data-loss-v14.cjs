...
const astRescans=[...astText.matchAll(/\b(?:ModelScanner|FormRequestScanner|ControllerScanner|ResourceScanner|RouteScanner)\.(?:scan|scanAsts|scanCanonicalAsts)\s*\(/g)].map(m=>({callee:m[0].replace(/\($/,''),line:line(astText,m.index)}));
add({type:'CANONICAL_AST_SELF_RESCAN',severity:'critical',status:astRescans.length?'PROVEN':'NOT_FOUND',count:astRescans.length,locations:astRescans,repair:'The canonical SourceAst producer itself must compose existing AST scanners without separately rebuilding semantic source data; strengthen the AST composition boundary instead of duplicating scans.'});
...
const report={schema:'routesync.data-loss-trace/v14',status:!proof.every(x=>x.found)?'UNPROVEN':astRescans.length?'SOURCE_AST_REBUILDS_SOURCE':sourceUses.length<=1?'SOURCE_AST_DEAD_AFTER_ASSIGNMENT':conditionLoss.length||targetedTernary.length?'PROVEN_LOSS':'LINEAGE_REVIEW',target:{repo,project,tsFiles:tsFiles.length,phpFiles:phpFiles.length},producerChain:proof,findings,boundaryInventory:{count:boundaries.length,samples:boundaries.slice(0,180)},metrics:{phpFiles:phpFiles.length,sourceAstUses:sourceUses.length,canonicalAstRescans:astRescans.length,freeDataBoundaries:freeData.length,duplicateNullability:duplicateVocabulary.length,nullableCalls:nullableCalls.length,splitSemanticAuthority:splitAuthority.length,projectionCandidates:projection.length,conditionLoss:conditionLoss.length,branchCollapse:branchCollapse.length,targetedTernary:targetedTernary.length,fallbackReclassification:fallbackReclassification.length,manifestFields:manifestParams.length},repairOrder:[
 {order:1,action:'Make canonical SourceAsts the actual origin-boundary input to existing scanners; stop independent projectRoot rescans after the canonical AST is available.'},

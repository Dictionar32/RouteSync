...
const completeModelValueProducerEvidence=highModelDataflowEvidence.filter(x=>
  ['VALUE_DECLARATION','VALUE_RETURN','FACTS_ASSIGNMENT','VALUE_CALL'].includes(x.kind) &&
  /CompleteLaravelSourceModel/.test(x.type||x.contextualType||x.parameterType||x.argumentType||'')
);
...
const out=abs('data-loss-trace-v50.json');fs.writeFileSync(out,JSON.stringify(report,null,2));
